...
namespace mithep 
{
  class TH2DAsymErr : public TH2D
  {
    public:
      ...
      Double_t  GetBinStatErrorHigh(Int_t b, Int_t c)    { return GetBinError(b,c,kStatErrHigh); }
      Double_t  GetSysErrorLow(Double_t x, Double_t y)   { return GetError(x,y,kSysErrLow);      }
      Double_t  GetBinSysErrorLow(Int_t b, Int_t c)      { return GetBinError(b,c,kSysErrLow);   }
      Double_t  GetSysErrorHigh(Double_t x, Double_t y)  { return GetError(x,y,kSysErrHigh);     }
      Double_t  GetBinSysErrorHigh(Int_t b, Int_t c)     { return GetBinError(b,c,kSysErrHigh);  }
